...
import { Store, ShieldCheck, ArrowRight, Lock, MapPin, UserRound } from 'lucide-react';
...
export const AdminPortal: React.FC = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    setLoginError('');
    try {
      const session = await api.loginAdmin(username, password);
      const allowedCafes = new Set(session.cafeSlugs || []);
      const allCafes = await api.getAllCafes();
      setCafes(allCafes.filter((cafe) => allowedCafes.has(cafe.slug)));
        ...
        {!isAuthenticated ? (
          ...
          <form onSubmit={handleLogin} className="glass-panel p-6 rounded-3xl border border-white/[0.08] space-y-5 shadow-2xl animate-fadeIn">
            ...
            <div className="space-y-1.5">
              <label className="text-[11px] font-bold text-gray-400 uppercase tracking-widest block">Identifiant</label>
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full bg-white/[0.04] text-sm text-white pl-10 pr-3.5 py-3.5 rounded-2xl border border-white/[0.08] focus:border-orange-500/50 focus:outline-none"
                />
              </div>
            </div>
