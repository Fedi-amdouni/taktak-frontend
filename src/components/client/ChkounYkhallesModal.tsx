...
export const ChkounYkhallesModal: React.FC<Props> = ({ isOpen, onClose, tableId }) => {
  ...
  useEffect(() => {
    ...
    return () => { void activeSocket?.disconnect(); setSocket(null); };
  }, [tableId, isOpen]);
  ...
  return <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/80 p-4 backdrop-blur-md">
    <div className="w-full max-w-sm overflow-hidden rounded-[28px] border border-orange-400/30 bg-[#10111b] shadow-2xl">
      <div className="flex items-center justify-between p-5"><div><p className="text-lg font-black text-white">Chkoun ykhalles ?</p><p className="text-xs text-gray-400">Le hasard décide pour la table.</p></div><button onClick={onClose} className="rounded-xl bg-white/5 p-2 text-gray-400 hover:text-white"><X className="h-5 w-5" /></button></div>
      <div className="mx-5 rounded-3xl border border-orange-400/20 bg-gradient-to-br from-orange-500/20 to-amber-400/5 p-6 text-center">
        <div className="mx-auto mb-2 text-amber-200">▼</div>
        <div style={{ background: wheel }} className={`mx-auto mb-4 flex h-40 w-40 items-center justify-center rounded-full border-8 border-white/80 text-4xl shadow-xl shadow-orange-500/30 ${spinning ? 'animate-spin' : ''}`}>🎯</div>
        {spinning ? <p className="animate-pulse text-xl font-black text-amber-200">La roue tourne…</p> : loser ? <GameResultBanner winnerName={loser} label="Résultat du tirage" detail={`C'est ${loser} qui paie la note !`} tone="amber" /> : <p className="text-sm font-semibold text-gray-300">Rejoignez puis lancez le tirage.</p>}
      </div>
      <div className="space-y-3 p-5">
        <div className="flex gap-2"><input value={name} onChange={event => setName(event.target.value)} onKeyDown={event => event.key === 'Enter' && join()} placeholder="Votre prénom" maxLength={32} className="min-w-0 flex-1 rounded-xl border border-white/10 bg-black/20 px-3 py-2.5 text-sm text-white outline-none focus:border-orange-400" /><button onClick={join} className="rounded-xl bg-white/10 px-3 text-xs font-bold text-white">Rejoindre</button></div>
        ...
        <button disabled={players.length === 0 || spinning} onClick={() => socket?.send('roulette/start')} className="flex w-full items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-orange-500 to-amber-500 py-3.5 text-sm font-extrabold text-white disabled:opacity-50"><Dices className="h-5 w-5" /> Lancer la roulette</button>
      </div>
    </div>
  </div>;
};
