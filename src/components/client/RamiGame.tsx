      ...
      {!started ? (
        ...
        <>
          ...
          <div className="rounded-3xl border border-white/10 bg-white/[.035] p-4">
            ...
            <div className="relative mx-auto mt-5 h-[176px] max-w-[350px] touch-pan-y" onDragOver={event => event.preventDefault()} onDrop={event => { event.preventDefault(); const sourceId = draggedCardId || event.dataTransfer.getData('text/plain'); if (sourceId) moveHandCard(sourceId); setDraggedCardId(null); }}>
              {hand.map((card, index) => (
                <div
                  key={card.id}
                  ...
                  className={`absolute bottom-0 left-1/2 cursor-grab rounded-xl transition duration-200 active:cursor-grabbing animate-card-deal ${draggedCardId === card.id ? 'opacity-45' : ''} ${dragOverCardId === card.id ? 'z-30 -translate-y-5 ring-2 ring-[#f1c777] ring-offset-2 ring-offset-[#17201d]' : ''}`}
                  style={{
                    zIndex: draggedCardId === card.id ? 40 : index + 1,
                    animationDelay: `${index * 80}ms`,
                    transform: `translateX(calc(-50% + ${(index - (hand.length - 1) / 2) * Math.max(17, Math.min(32, 286 / Math.max(1, hand.length - 1)))}px)) translateY(${Math.abs(index - (hand.length - 1) / 2) * 1.25}px) rotate(${(index - (hand.length - 1) / 2) * Math.max(-3.6, Math.min(3.6, 34 / Math.max(1, hand.length - 1)))}deg)`,
                  } as React.CSSProperties}
                >
                  <CardFace card={card} selected={selectedIds.includes(card.id)} disabled={!myTurn || !hasDrawn} onClick={myTurn && hasDrawn ? () => toggleCard(card) : undefined} />
                </div>
              ))}
            </div>
