...
export const ConnectFourGame: React.FC<Props> = ({ tableId, onBack }) => {
  ...
  return <section className="mx-auto max-w-md p-4 pb-24 space-y-4">
    ...
    <div className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-center text-sm font-bold text-white"><Users className="mr-1 inline h-4 w-4 text-gray-400" />{status}</div>
    {(winner || draw) && <button onClick={() => socket.current?.send('connect-four/replay')} className="flex w-full items-center justify-center gap-2 rounded-2xl bg-white/10 py-3 text-sm font-bold text-white hover:bg-white/15"><RotateCcw className="h-4 w-4" /> Rejouer</button>}
  </section>;
};
