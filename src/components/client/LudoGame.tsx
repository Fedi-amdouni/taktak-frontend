  ...
  useEffect(() => {
    const handleEvent = (event: GameEvent) => {
      ...
      if (changedTokens.length) {
        ...
        moveTimerRef.current = window.setTimeout(() => setMovingTokens([]), 560);
      }
      if (event.ludoDice !== null && event.ludoDice !== undefined) animateDice();
      setState(event);
    };
    ...
    {!state.ludoStarted ? <div className="space-y-3 rounded-3xl border border-white/10 bg-[#11131d] p-4">
      ...
      <div className="relative mx-auto w-full overflow-hidden rounded-2xl border-4 border-white bg-white shadow-[0_24px_55px_rgba(0,0,0,.5)]" style={{ aspectRatio: '259 / 267' }}>
        ...
        {players.flatMap((player, playerIndex) => (state.ludoTokens?.[player.id] || []).map((progress, tokenIndex) => {
          ...
          const movable = isMine && myTurn && !state.ludoCanRoll && !winner && !isRolling;
          const tokenKey = `${player.id}-${tokenIndex}`;
          return <button key={tokenKey} disabled={!movable} onClick={() => socket.current?.send('ludo/move', { playerId: gamePlayerId, tokenIndex })} aria-label={`Pion ${tokenIndex + 1} de ${player.name}`} className={`absolute z-20 grid -translate-x-1/2 -translate-y-1/2 place-items-center rounded-full border-2 border-white shadow-[0_3px_5px_rgba(0,0,0,.5)] transition-[left,top] duration-500 ease-in-out ${movable ? 'h-[7.5%] w-[7.5%] animate-pulse cursor-pointer ring-2 ring-slate-900' : 'h-[6.5%] w-[6.5%]'} ${movingTokens.includes(tokenKey) ? 'animate-token-hop' : ''}`} style={{ left: `${((column + .5) / 15) * 100 + (tokenIndex - 1.5) * .45}%`, top: `${((row + .5) / 15) * 100 + (tokenIndex - 1.5) * .35}%`, backgroundColor: colors[playerIndex] }}><span className="h-2/5 w-2/5 rounded-full bg-white/60" /></button>;
        }))}
        <div className={`absolute left-1/2 top-1/2 z-30 grid h-[15%] w-[15%] -translate-x-1/2 -translate-y-1/2 place-items-center rounded-xl border-2 border-white/80 bg-slate-900/90 text-3xl text-white shadow-xl ${isRolling ? 'animate-dice-roll' : ''}`}>{isRolling ? diceFace[rollingFace] : state.ludoDice ? diceFace[state.ludoDice] : '🎲'}</div>
      </div>
      ...
      {winner ? <GameResultBanner winnerName={winner} tone="cyan" actionLabel="Rejouer la partie" onAction={() => socket.current?.send('ludo/start')} /> : <div className="rounded-2xl bg-white/5 p-3"><p className="mb-3 text-center text-xs font-black text-yellow-300">{myTurn ? (isRolling ? 'Le dé tourne…' : state.ludoCanRoll ? 'Lance le dé' : 'Choisis un pion lumineux') : 'En attente du joueur actif'}</p><button disabled={!myTurn || !state.ludoCanRoll || isRolling} onClick={rollDice} className="flex w-full items-center justify-center gap-2 rounded-2xl bg-yellow-300 py-3 font-black text-slate-950 disabled:opacity-40"><Dices className="h-5 w-5" /> Lancer le dé</button></div>}
    </>}
  </section>;
};
