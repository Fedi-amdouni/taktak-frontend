...
import { ChefHat, Volume2, VolumeX, RefreshCw, Sparkles, Zap, Bell, CheckCircle, Sun, Activity, MapPin, Filter, Lock, Calendar, Archive, Flame, UtensilsCrossed, Footprints, Layout, List } from 'lucide-react';
...
export const KanbanBoard: React.FC<KanbanBoardProps> = ({ cafeSlug }) => {
  ...
  const handleReleaseTable = async (orderId: string) => {
    try {
      const updated = await api.updateOrderStatus(orderId, 'ARCHIVED');
      if (updated && updated.id) {
        setOrders((prev) =>
          (prev || [])
            .filter(Boolean)
  ...
  const pickedUpCount = zonedOrders.filter((o) => o.status === 'PICKED_UP').length;
  ...
  const filterTabs = [
    ...
    { key: 'PICKED_UP', label: '🏃‍♂️ En Livraison', count: pickedUpCount, dotColor: 'bg-blue-400' },
  ...
  return (
    <div className="min-h-screen bg-[#08090e] text-white p-4 sm:p-6 space-y-6 relative">
      ...
      {/* Header Tablet Bar */}
      <header className="glass-panel p-4 rounded-2xl flex flex-wrap items-center justify-between gap-4 relative z-10">
